...
class ImageSnapOverlayView: UIView {
    ...
    private let imageSnapOverLayerCornerWidth = CGFloat(20.0)
    
    var gridLineNumberType: GridLineNumberType = .imageSnap {
        didSet {
            setupGridLines()
            layoutGridLines()
        }
    ...
    private func layoutCornerLines() {
        ...
        let topLeftHorizonalLayerFrame = CGRect(x: -borderThickness, y: -borderThickness, width: imageSnapOverLayerCornerWidth, height: borderThickness)
        let topLeftVerticalLayerFrame = CGRect(x: -borderThickness, y: -borderThickness, width: borderThickness, height: imageSnapOverLayerCornerWidth)
                
        let horizontalDistanceForHCorner = bounds.width + 2 * borderThickness - imageSnapOverLayerCornerWidth
        ...
        let veticalDistanceForVCorner = bounds.height + 2 * borderThickness - imageSnapOverLayerCornerWidth
        
        for (i, line) in corner.enumerated() {
            let lineType: CornerLineType = ImageSnapOverlayView.CornerLineType(rawValue: i) ?? .topLeftVertical
            switch lineType {
    ...
    func handleEdgeTouched(with tappedEdge: ImageSnapViewOverlayEdge) {
...
extension ImageSnapOverlayView {
