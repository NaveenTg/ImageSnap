...
protocol ImageSnapMaskProtocol where Self: UIView {
    var imageSnapShapeType: ImageSnapShapeType { get set }
...
extension ImageSnapMaskProtocol {
    func initialize() {
        setInitialFrame()
        setMask()
    }
    ...
    func adaptMaskTo(match imageSnapRect: CGRect) {
        let scaleX = imageSnapRect.width / minOverLayerUnit
        let scaleY = imageSnapRect.height / minOverLayerUnit
        
        transform = CGAffineTransform(scaleX: scaleX, y: scaleY)
        
        self.frame.origin.x = imageSnapRect.midX - self.frame.width / 2
        self.frame.origin.y = imageSnapRect.midY - self.frame.height / 2
    }
